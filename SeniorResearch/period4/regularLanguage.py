...
def callWithExample3():
    ...
    wordLen = 1000
    task = int(input("Type 1 for a graph of density trend. \n 2 for difference in time for computation."))
    if task == 1:
        wordLen = 100000
        lowLogValue = 99950
        highLogValue = 99999
        logArray = dfa3.LogDensityRecurrence(wordLen, lowLogValue, highLogValue)
        print len(logArray)
        print logArray
    ...
    #matplotlib code
        ...
        matrixDensity = dfa3.ComputeDensityMatrix(wordLen +1)
        matrixTimeEnd = time.time()
        recurrenceTimeStart = time.time()
        recurrenceDensity = dfa3.ComputeDensityRecurrence(wordLen)
